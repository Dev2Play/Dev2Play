...
router.get('/perfil', isLoggedIn, async (req, res) => {
    ...
    if (equipo.length == 0) {
        res.render('../views/partials/perfil/perfil', { usuario: req.user, archivo: public, roles: rol, photo: foto_perfil[0].foto });
    }

    if (rol.length == 0) {
        res.render('../views/partials/perfil/perfil', { usuario: req.user, archivo: public, equipos: equipo[0].titulo, photo: foto_perfil[0].foto });
    }

    else {
        res.render('../views/partials/perfil/perfil', { usuario: req.user, roles: rol, equipos: equipo[0].titulo, archivo: public, photo: foto_perfil[0].foto });
    }
